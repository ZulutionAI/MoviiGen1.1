...
def main(args):
    ...
    vae, autocast_type, fps = [None,torch.bfloat16,16]
    # vae.enable_tiling()
    ...
    for _, data in tqdm(enumerate(train_dataloader), disable=local_rank != 0):
        with torch.inference_mode():
            with torch.autocast("cuda", dtype=autocast_type):
                outputs = text_encoder(data["caption"],device)
                prompt_embeds = outputs
                padded_embeds = []
                padded_masks = []
                max_seq_len = 256
                for embd in prompt_embeds:
                    seq_len, dim = embd.shape
                    pad_len = max_seq_len - seq_len
                    padded_embd = F.pad(embd, (0, 0, 0, pad_len), value=0)  # 输出形状 [max_seq_len, dim]
                    breakpoint()
                    padded_embeds.append(padded_embd)
                    mask = torch.ones(seq_len, dtype=torch.long)
                    padded_mask = F.pad(mask, (0, pad_len), value=0)  # 输出形状 [max_seq_len]
                    padded_masks.append(padded_mask)
                prompt_embeds = padded_embeds
                prompt_attention_mask = padded_masks
                # prompt_attention_mask = [torch.ones(embd.size()) for embd in prompt_embeds]
