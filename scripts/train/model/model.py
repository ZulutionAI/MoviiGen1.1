...
class Head(nn.Module):
    ...
    def __init__(self, dim, out_dim, patch_size, eps=1e-6):
        ...
        # layers
        out_dim = math.prod(patch_size) * out_dim + 1
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        breakpoint()
        print(f'rank {dist.get_rank()}, inside x{x[0].shape}')  # [1, 5120, 20, 30, 52] [B, C, F, H, W]
        ...
        # i = 0
        # [1, 5120, 20, 30, 52] [B, C, F, H, W] -> [1, 31200, 5120] [B, HWF, C]
        print(f'rank {dist.get_rank()}, attn x{x.shape}')
